...
class PnLCalculatorTest {
    ...
    void testCalculatePnLForDifferentCurrencies() {
        ...
        // PnL calculation doesn't consider currency, so both should be equal
        assertTrue(eurPnL > 0);
        assertEquals(usdPnL, eurPnL, 0.01);
    }
    ...
    void testCalculatePnLForDifferentPriceLevels() {
        ...
        // With price 50.0, P&L = 1000000.0 * (50.0 - 100.0) / 100.0 = -500000.0 (negative)
        ...
        // With price 200.0, P&L = 1000000.0 * (200.0 - 100.0) / 100.0 = 1000000.0 (positive)
    ...
    void testMultipleContractPnLCalculations() {
        // Calculate P&L for equity swap contract (only supported type)
        double equityPnL = pnLCalculator.calculatePnL(equitySwapContract, marketData);

        // Equity swap should calculate successfully
        assertTrue(equityPnL > 0);

        // Interest rate swap and bond should throw exceptions
