...
-- Natural Key: (contract_id, from_date, to_date, calculation_type)
...
CREATE TABLE calculation_requests (
    ...
    calculation_type VARCHAR(20) NOT NULL, -- Natural key component
    from_date DATE NOT NULL, -- Natural key component
    to_date DATE NOT NULL, -- Natural key component
    ...
    CONSTRAINT uk_calculation_natural_key UNIQUE (contract_id, from_date, to_date, calculation_type)
) WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = calculation_requests_history));

-- Indexes for calculation_requests table
CREATE INDEX idx_calc_requests_contract ON calculation_requests(contract_id);
...
CREATE INDEX idx_calc_requests_natural_key ON calculation_requests(contract_id, from_date, to_date, calculation_type);
...
CREATE INDEX idx_settlements_type ON settlement_instructions(settlement_type);
CREATE INDEX idx_settlements_natural_key ON settlement_instructions(contract_id, cash_flow_id, settlement_date, settlement_type);
CREATE INDEX idx_settlements_actual_date ON settlement_instructions(actual_settlement_date);
