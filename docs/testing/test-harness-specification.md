...
### 1. Complete Cash Flow Input Data

#### Cash Flow Request Content Example
```json
{
...
```java
@Test
void testValidCashFlowRequest() {
    // Given: Complete cash flow request with all components
...
void testMissingRequiredFields() {
    // Given: Cash flow request missing contracts
    CashFlowRequestContent request = createIncompleteCashFlowRequest();
...
void testInvalidDateRange() {
    // Given: Cash flow request with invalid date range
...
@Test
void testMarketDataStrategyValidation() {
    // Given: Cash flow request with different market data strategies
    CashFlowRequestContent hybridRequest = createHybridMarketDataRequest();
    CashFlowRequestContent selfContainedRequest = createSelfContainedMarketDataRequest();
    
    // When: Validate requests
    ValidationResult hybridResult = validator.validateCashFlowRequest(hybridRequest);
    ValidationResult selfContainedResult = validator.validateCashFlowRequest(selfContainedRequest);
    
    // Then: Verify both strategies are valid
    assertTrue(hybridResult.isValid());
...
void testValidContractCreation() {
    // Given: Contract with valid equity and interest legs
    Contract contract = createValidContract();
    
    // When: Validate contract
...
@Test
void testInvalidContractData() {
    // Given: Contract with missing required fields
    Contract contract = createInvalidContract();
    ...
    assertFalse(result.isValid());
    assertTrue(result.getErrors().stream()
        .anyMatch(error -> error.getCode().equals("MISSING_EQUITY_LEG")));
}

@Test
void testCurrencyMismatch() {
    // Given: Contract with mismatched currencies
...
void testPositionCreation() {
    ...
    // Then: Verify position creation
    assertNotNull(result.getPositionId());
    assertEquals("ACTIVE", result.getStatus());
    assertEquals(1000.0, result.getTotalQuantity(), 0.01);
}

@Test
void testPositionUpdates() {
    // Given: Existing position with updates
    Position existingPosition = createExistingPosition();
    PositionUpdate update = createPositionUpdate();
    
    // When: Update position
    Position result = positionService.updatePosition(existingPosition.getPositionId(), update);
    
    // Then: Verify position update
    assertEquals(1500.0, result.getTotalQuantity(), 0.01);
    assertEquals("ACTIVE", result.getStatus());
}
```
...
void testLotCreation() {
    ...
    // Then: Verify lot creation
    assertNotNull(result.getLotId());
    assertEquals("ACTIVE", result.getStatus());
    assertEquals(1000.0, result.getQuantity(), 0.01);
    assertEquals(120.00, result.getCostPrice(), 0.01);
}

@Test
void testUnwindingMethods() {
    // Given: Position with multiple lots
    List<Lot> lots = createMultiLotPosition();
    
    // When: Apply different unwinding methods
    UnwindingResult lifoResult = lotService.unwindLots(lots, "LIFO", 500.0);
    UnwindingResult fifoResult = lotService.unwindLots(lots, "FIFO", 500.0);
    
    // Then: Verify different unwinding results
    assertNotEquals(lifoResult.getRealizedPnL(), fifoResult.getRealizedPnL());
}
```
...
```java
@Test
void testInterestPaymentSchedule() {
    // Given: Interest payment schedule
    PaymentSchedule schedule = createInterestPaymentSchedule();
    
    // When: Validate schedule
    ValidationResult result = validator.validatePaymentSchedule(schedule);
    
    // Then: Verify validation passes
    assertTrue(result.isValid());
    assertEquals("INTEREST_PAYMENT", schedule.getScheduleType());
    assertEquals("QUARTERLY", schedule.getFrequency());
}

@Test
void testDividendPaymentSchedule() {
    // Given: Dividend payment schedule
    PaymentSchedule schedule = createDividendPaymentSchedule();
    ...
    // Then: Verify validation passes
    assertTrue(result.isValid());
    assertEquals("DIVIDEND_PAYMENT", schedule.getScheduleType());
    assertEquals("QUARTERLY", schedule.getFrequency());
}
```
