...
start_test_infrastructure() {
    ...
    while [ $timeout -gt 0 ]; do
        if docker-compose -f docker-compose.test.yml exec sqlserver-test /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P TestPassword123! -Q "SELECT 1" > /dev/null 2>&1; then
            break
        fi
        sleep 5
        timeout=$((timeout - 5))
