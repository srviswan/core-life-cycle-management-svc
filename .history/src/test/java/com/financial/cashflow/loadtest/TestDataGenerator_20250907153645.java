...
public class TestDataGenerator {
    ...
    private static CashFlowRequest.MarketDataContainer createMarketData() {
        List<CashFlowRequest.SecurityData> securities = new ArrayList<>();
        List<CashFlowRequest.RateData> rates = new ArrayList<>();
        List<CashFlowRequest.DividendData> dividends = new ArrayList<>();
        ...
        for (int i = 0; i < symbols.length; i++) {
            ...
            CashFlowRequest.SecurityData security = CashFlowRequest.SecurityData.builder()
                .symbol(symbol)
            ...
            // Dividend
            List<CashFlowRequest.Dividend> dividendInfos = new ArrayList<>();
            dividendInfos.add(CashFlowRequest.Dividend.builder()
                .exDate(LocalDate.now().minusDays(5))
                .amount(1.0 + (i * 0.1))
                .currency("USD")
                .build());
            
            CashFlowRequest.DividendData dividend = CashFlowRequest.DividendData.builder()
                .symbol(symbol)
        ...
        CashFlowRequest.RateData rate = CashFlowRequest.RateData.builder()
            .index("SOFR")
            .baseRate(5.25)
            .baseDate(LocalDate.now().minusDays(1))
            .changes(rateChanges)
