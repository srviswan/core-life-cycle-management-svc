...
class DividendCalculatorWithDetailsTest {
    ...
    void testCalculateDividendsWithDetails_MultipleDividends() {
        ...
        assertEquals(150.0, taxInfo2.getWithholdingTaxAmount(), 0.01);
    ...
    void testCalculateDividendsWithDetails_DifferentTaxJurisdictions() {
        ...
        WithholdingTaxInfo taxInfo = result.getWithholdingTaxDetails().get(0);
        assertEquals("EU", taxInfo.getTaxJurisdiction()); // EUR currency maps to EU jurisdiction
        assertEquals("EUR", taxInfo.getCurrency());
    }
    ...
    void testCalculateDividendsWithDetails_AllWithholdingTreatments() {
        ...
        // Then
        assertNotNull(result);
        assertEquals(3, result.getWithholdingTaxDetails().size());
        ...
        // Verify NET_AMOUNT treatment
        WithholdingTaxInfo netAmountInfo = result.getWithholdingTaxDetails().get(1);
        assertEquals(WithholdingTaxInfo.WithholdingTreatment.NET_AMOUNT, netAmountInfo.getWithholdingTreatment());
        assertEquals(1500.0, netAmountInfo.getGrossDividendAmount(), 0.01);
        assertEquals(0.0, netAmountInfo.getWithholdingTaxAmount(), 0.01); // No additional tax
        assertEquals(1500.0, netAmountInfo.getNetDividendAmount(), 0.01);

        // Verify NO_WITHHOLDING treatment
        WithholdingTaxInfo noWithholdingInfo = result.getWithholdingTaxDetails().get(2);
        assertEquals(WithholdingTaxInfo.WithholdingTreatment.NO_WITHHOLDING, noWithholdingInfo.getWithholdingTreatment());
        assertEquals(1000.0, noWithholdingInfo.getGrossDividendAmount(), 0.01);
        assertEquals(0.0, noWithholdingInfo.getWithholdingTaxAmount(), 0.01);
        assertEquals(1000.0, noWithholdingInfo.getNetDividendAmount(), 0.01);
    }
}
