...
class CashFlowIntegrationTest {
    ...
    static void configureProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("spring.datasource.url", () -> "jdbc:sqlserver://localhost:1434;databaseName=cashflow_test;encrypt=true;trustServerCertificate=true");
    ...
    @Test
    void shouldVerifyControllerIsRegistered() {
        // This test verifies that the controller is properly registered
        assertThat(applicationContext).isNotNull();
        
        // Check if the controller bean is registered
        assertThat(applicationContext.containsBean("cashFlowController")).isTrue();
        
        // Check if the service beans are registered
        assertThat(applicationContext.containsBean("cashFlowCalculationService")).isTrue();
        assertThat(applicationContext.containsBean("cashFlowValidatorImpl")).isTrue();
        assertThat(applicationContext.containsBean("cashFlowEngineImpl")).isTrue();
        assertThat(applicationContext.containsBean("settlementServiceImpl")).isTrue();
        assertThat(applicationContext.containsBean("eventPublisherImpl")).isTrue();
        assertThat(applicationContext.containsBean("cacheServiceImpl")).isTrue();
    }

    @Test
    void shouldCheckActuatorMappings() {
        // Check what endpoints are actually registered
        ResponseEntity<String> response = restTemplate.getForEntity(
            "http://localhost:" + port + "/actuator/mappings",
            String.class
        );
        
        // The test should pass regardless of the response to see what's registered
