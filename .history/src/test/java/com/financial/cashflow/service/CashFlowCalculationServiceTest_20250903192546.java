...
class CashFlowCalculationServiceTest {
    ...
    void shouldCalculateCashFlowsSuccessfully() {
        ...
        verify(engine).calculateCashFlows(request);
        verify(settlementService).generateSettlementInstructions(expectedCashFlows, request);
        verify(eventPublisher).publishCashFlowCalculated(response);
        verify(cacheService).put(anyString(), response);
    }
    ...
    void shouldReturnCachedResultWhenAvailable() {
        ...
        // Then
        assertNotNull(response);
        assertTrue(response.getSummary().getCacheHit());
    ...
    void shouldThrowValidationExceptionWhenRequestIsInvalid() {
        ...
        assertThrows(CashFlowCalculationException.class, () -> future.join());
    ...
    void shouldCalculateSummaryCorrectly() {
        ...
        assertFalse(summary.getCacheHit());
    }
