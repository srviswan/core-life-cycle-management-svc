...
public class CalculationEngine {
     ...
     * Creates partitions by contract + position combinations
     */
    private List<CashFlowResponse.ContractResult> processContractPositionsParallel(List<CashFlowRequest.ContractPosition> contractPositions, 
        ...
        List<CashFlowResponse.ContractResult> allResults = new ArrayList<>();
        
        for (CashFlowRequest.ContractPosition contractPosition : contractPositions) {
            log.info("Processing contract position: {} with {} positions", 
                contractPosition.getContractId(), contractPosition.getPositions().size());
            
            // Process each position within the contract in parallel
            List<CashFlowResponse.ContractResult> positionResults = contractPosition.getPositions()
                .parallelStream()
                .map(position -> {
                    try {
                        log.info("Processing position: {} in contract: {}", 
                            position.getPositionId(), contractPosition.getContractId());
                        return calculateContractPosition(contractPosition, position, request, marketData);
                    } catch (Exception e) {
            ...
            allResults.addAll(positionResults);
        }
    ...
    private List<CashFlowResponse.CashFlow> createPositionLotLevelCashFlows(CashFlowRequest.ContractPosition contractPosition, 
        ...
        for (CashFlowRequest.Lot lot : position.getLots()) {
            double lotRatio = lot.getQuantity() / totalQuantity;
