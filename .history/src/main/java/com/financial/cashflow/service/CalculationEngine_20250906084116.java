...
public class CalculationEngine {
    ...
    private CashFlowResponse.ContractResult calculateContract(CashFlowRequest.Contract contract, 
        ...
        try {
            ...
            log.debug("Checking lots for contract {}: lots={}, size={}", 
                contract.getContractId(), request.getLots(), 
                request.getLots() != null ? request.getLots().size() : 0);
            
            if (request.getLots() != null && !request.getLots().isEmpty()) {
                log.debug("Creating lot-level cash flows for contract: {}", contract.getContractId());
                lotCashFlows = createLotLevelCashFlows(contract, request, marketData, pnl, interest, dividends);
                log.debug("Created {} lot-level cash flows for contract: {}", lotCashFlows.size(), contract.getContractId());
            } else {
                log.debug("No lots available for contract: {}", contract.getContractId());
            }
    ...
    private List<CashFlowResponse.CashFlow> createLotLevelCashFlows(CashFlowRequest.Contract contract, 
        ...
        log.debug("Creating lot-level cash flows for contract: {}, total lots: {}", 
        ...
        log.debug("Found {} lots for contract: {}", contractLots.size(), contract.getContractId());
    ...
    private double getInterestRate(CashFlowRequest.Contract contract, MarketData marketData) {
        return marketData.getRates().stream()
            .filter(rate -> contract.getIndex().equals(rate.getIndex()))
            .findFirst()
            .map(MarketData.RateData::getBaseRate)
            .orElse(0.0);
    }
}
