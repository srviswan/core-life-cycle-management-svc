...
public class DividendCalculator {
    ...
    private double calculateEquitySwapDividends(CashFlowRequest.Contract contract, 
        ...
        return dividends.stream()
            ...
            .mapToDouble(dividend -> {
                ...
                return applyWithholdingTax(dividend, grossAmount);
            })
            .sum();
    }
    ...
    private WithholdingTaxInfo createWithholdingTaxInfo(CashFlowRequest.Contract contract,
        ...
        // For TAX_CREDIT treatment, the withholding tax amount is the actual tax that can be claimed as credit
        if (dividend.getWithholdingTreatment() == Dividend.WithholdingTreatment.TAX_CREDIT) {
            withholdingTaxAmount = grossDividendAmount * (withholdingTaxRate / 100.0);
        } else {
            withholdingTaxAmount = grossDividendAmount - netDividendAmount;
        }
    ...
    public double calculateDividendsForPosition(CashFlowRequest.ContractPosition contractPosition, 
                                              ...
                                              MarketData marketData, 
                                              LocalDate calculationDate) {
        log.debug("Calculating position-based dividends for contract: {}, position: {}", 
