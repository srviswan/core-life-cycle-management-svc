...
-- Settlement Instructions table (updated to match API specification)
CREATE TABLE settlement_instructions (
    settlement_id NVARCHAR(50) NOT NULL,
    contract_id NVARCHAR(50) NOT NULL,
    cash_flow_id NVARCHAR(50) NOT NULL,
    settlement_date DATE NOT NULL,
    settlement_type NVARCHAR(20) NOT NULL,
    counterparty NVARCHAR(100) NOT NULL,
    amount DECIMAL(18,2) NOT NULL,
    currency NVARCHAR(3) NOT NULL,
    status NVARCHAR(20) NOT NULL DEFAULT 'PENDING',
    ...
    CONSTRAINT PK_settlement_instructions PRIMARY KEY (settlement_id),
    CONSTRAINT UQ_settlement_natural_key UNIQUE (contract_id, cash_flow_id, settlement_date, settlement_type)
);
GO

-- Calculation Requests audit trail table
CREATE TABLE calculation_requests (
    request_id NVARCHAR(50) NOT NULL,
    contract_id NVARCHAR(50) NOT NULL,
...
-- Create withholding_tax_details table for tax utility reporting
CREATE TABLE withholding_tax_details (
    ...
    ex_date DATE,
    payment_date DATE,
    gross_dividend_amount DECIMAL(18,2) NOT NULL,
    withholding_tax_rate DECIMAL(5,2),
    withholding_tax_amount DECIMAL(18,2),
    net_dividend_amount DECIMAL(18,2) NOT NULL,
    withholding_treatment NVARCHAR(50),
    tax_jurisdiction NVARCHAR(50),
...
CREATE INDEX IX_settlement_instructions_type ON settlement_instructions (settlement_type);
CREATE INDEX IX_settlement_instructions_date ON settlement_instructions (settlement_date);

CREATE INDEX IX_calculation_requests_contract ON calculation_requests (contract_id);
CREATE INDEX IX_calculation_requests_status ON calculation_requests (status);
CREATE INDEX IX_calculation_requests_type ON calculation_requests (calculation_type);
CREATE INDEX IX_calculation_requests_date ON calculation_requests (from_date, to_date);

CREATE INDEX IX_withholding_tax_contract_id ON withholding_tax_details (contract_id);
CREATE INDEX IX_withholding_tax_calculation_date ON withholding_tax_details (calculation_date);
CREATE INDEX IX_withholding_tax_tax_jurisdiction ON withholding_tax_details (tax_jurisdiction);
CREATE INDEX IX_withholding_tax_tax_utility_reference ON withholding_tax_details (tax_utility_reference);
CREATE INDEX IX_withholding_tax_ex_date ON withholding_tax_details (ex_date);
CREATE INDEX IX_withholding_tax_payment_date ON withholding_tax_details (payment_date);
